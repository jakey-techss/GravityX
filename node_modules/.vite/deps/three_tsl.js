...
var NodeMaterialObserver = class {
  ...
  equals(renderObject, lightsData, renderId) {
    ...
    if (materialData._renderId !== renderId) {
      ...
      for (const property3 in materialData) {
        ...
        if (property3 === "_renderId") continue;
        if (property3 === "_equal") continue;
  ...
  needsRefresh(renderObject, nodeFrame) {
    ...
    const notEqual3 = this.equals(renderObject, lightsData, renderId) !== true;
    return notEqual3;
  }
};
var IGNORED_FILES = [
  ...
  *getChildren() {
  ...
  getNodeType(builder, output3 = null) {
    ...
    if (output3 !== null) {
      ...
      type = nodeData.typeFromOutput[output3];
      if (type === void 0) {
        type = this.generateNodeType(builder, output3);
        nodeData.typeFromOutput[output3] = type;
      }
    } else {
      type = nodeData.type;
      if (type === void 0) {
   ...
   * @return {string} The type of the node.
   */
  generateNodeType(builder, output3 = null) {
    const nodeProperties = builder.getNodeProperties(this);
    if (nodeProperties.outputNode) {
      return nodeProperties.outputNode.getNodeType(builder, output3);
    }
...
var frameGroup = sharedUniformGroup("frame", 0, NodeUpdateType.FRAME);
var renderGroup = sharedUniformGroup("render", 0, NodeUpdateType.RENDER);
var objectGroup = uniformGroup("object", 1, NodeUpdateType.OBJECT);
...
var AssignNode = class extends TempNode {
  ...
  generateNodeType(builder, output3) {
...
var sinh = nodeProxyIntent(MathNode, MathNode.SINH).setParameterLength(1);
var cos = nodeProxyIntent(MathNode, MathNode.COS).setParameterLength(1);
var cosh = nodeProxyIntent(MathNode, MathNode.COSH).setParameterLength(1);
var tan = nodeProxyIntent(MathNode, MathNode.TAN).setParameterLength(1);
var tanh = nodeProxyIntent(MathNode, MathNode.TANH).setParameterLength(1);
var asin = nodeProxyIntent(MathNode, MathNode.ASIN).setParameterLength(1);
var asinh = nodeProxyIntent(MathNode, MathNode.ASINH).setParameterLength(1);
var acos = nodeProxyIntent(MathNode, MathNode.ACOS).setParameterLength(1);
var acosh = nodeProxyIntent(MathNode, MathNode.ACOSH).setParameterLength(1);
var atan = nodeProxyIntent(MathNode, MathNode.ATAN).setParameterLength(1, 2);
var atanh = nodeProxyIntent(MathNode, MathNode.ATANH).setParameterLength(1);
...
var remap = Fn(([node, inLowNode, inHighNode, outLowNode = float(0), outHighNode = float(1), doClamp = bool(false)]) => {
  ...
  generate(builder, output3) {
    ...
    if (/^sampler/.test(output3)) {
      ...
      if (propertyName === void 0) {
        ...
        if (finalDepthSnippet === null && texture3.isArrayTexture && this.isTexture3DNode !== true) {
          finalDepthSnippet = "0";
        }
        const nodeVar = builder.getVarFromNode(this);
        propertyName = builder.getPropertyName(nodeVar);
        let snippet2 = this.generateSnippet(builder, textureProperty, uvSnippet, levelSnippet, biasSnippet, finalDepthSnippet, compareSnippet, gradSnippet, offsetSnippet);
...
var materialEnvRotation = uniform(new Matrix4()).onReference(function(frame) {
  ...
  if (rotation) {
    _m1$1.makeRotationFromEuler(rotation).transpose();
  } else {
    _m1$1.identity();
  }
  return _m1$1;
...
var ViewportTextureNode = class extends TextureNode {
  ...
  updateReference(frame) {
    ...
    const reference3 = renderTarget ? renderTarget : canvasTarget;
    this.value = this.getTextureForReference(reference3);
    return this.value;
  }
  updateBefore(frame) {
    const renderer = frame.renderer;
    ...
    } else if (reference3.getDrawingBufferSize) {
      reference3.getDrawingBufferSize(_size$5);
    } else {
      _size$5.set(reference3.width, reference3.height);
    }
    const framebufferTexture = this.getTextureForReference(reference3);
...
var viewportDepthTexture = nodeProxy(ViewportDepthTextureNode).setParameterLength(0, 3);
...
var ShadowNode = class extends ShadowBaseNode {
  ...
  setupShadow(builder) {
    ...
    if (!renderer.highPrecision || builder.material.receivedShadowPositionNode || builder.context.shadowPositionWorld) {
      ...
      const highpShadowModelMatrix = uniform("mat4").onObjectUpdate(({ object }, self2) => {
        return self2.value.multiplyMatrices(shadowMatrix.value, object.matrixWorld);
      });
...
var asin2 = TSL.asin;
var asinh2 = TSL.asinh;
var assign2 = TSL.assign;
var atan2 = TSL.atan;
var atanh2 = TSL.atanh;
